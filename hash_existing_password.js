...
async function hashExistingPassword() {
  try {
    ...
    const [rows] = await pool.query('SELECT password FROM users WHERE id = 67'); // Assuming the first user has ID 1
    ...
    await pool.query('UPDATE users SET password = ? WHERE id = 67', [hashedPassword]);
